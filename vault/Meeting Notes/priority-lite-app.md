...
## Open Questions
- **Supabase project מושהה (pause) — 06-25**: הפרויקט `Priority_lite` (sxjizqkeirvnsgxpixcc) הושהה אחרי 7 ימי חוסר-פעילות. login לפרודקשן שבור עד ביטול השהיה מה-dashboard. אין השפעה על נתוני פריוריטי. חוסר-הפעילות הגיוני: session cookie ל-7 ימים → דיווחי שעות לא נוגעים ב-Supabase. **בזמן ביטול ההשהיה — לסובב את `SUPABASE_SERVICE_KEY` שדלף.**
