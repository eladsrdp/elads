...
- **כתיבה לפריוריטי — חוסם פעיל**: נתיב ה-POST תוקן (תת-טופס `ZRDP_DOCUMENTS_p(DOCNO,TYPE='p')/ZRDP_TRANSORDER_qp_SUBFORM`, מק"ט חובה `ש'ע`), אך נדחה ב"אין לשנות תאור מוצר" — המק"ט 'ש'ע' לא מסומן "ניתן לשנות תאור" במאפיינים נוספים למוצר. צריך החלטת המשתמש: לסמן את המק"ט, או מק"ט חלופי, או שלבי POST/PATCH. (שורות היסטוריות כן עם PDES חופשי — היה אפשרי בעבר.)
