...
## Overview
אפליקציית web ‏(PWA, עברית RTL) בתיקייה `priority-lite/` שעוטפת את Priority ERP בממשק מהיר: טיימר, דיווחי שעות ידניים, סיכומים (יום/שבוע/חודש לפי פרויקט→משימה), וחיפוש משימות. ארכיטקטורה: monorepo ‏(npm workspaces) עם `shared/` (טיפוסים), `server/` (Hono + Supabase, אימות OTP במייל לפי whitelist טלפונים, JWT cookie ל-7 ימים, adapter מבודד מול Priority OData עם mock לפיתוח), ו-`client/` (React 19 + Vite 8 + Tailwind v4 + Dexie). דיווחים נשמרים כטיוטות מקומיות ונשלחים לפריוריטי רק אחרי אישור המשתמש (תוצאה פר-פריט, שגיאות עם retry). שכבת actions עם zod schemas מוכנה לחיבור צ'אט-LLM בשלב 3. **סטטוס: בפרודקשן ב-Vercel** (`priority-lite-one.vercel.app`) — PRIORITY_MODE=real, EMAIL_MODE=resend, aiEnabled. Vercel Build Output API עם esbuild CJS bundle; `api-src/index.ts` מייצא `getRequestListener(app.fetch)` כ-Nodejs handler. כתיבה לפריוריטי עובדת ואומתה E2E.

## Open Questions
- **Supabase project מושהה (pause) — 06-25, עדיין דורש unpause ידני**: הפרויקט `Priority_lite` (sxjizqkeirvnsgxpixcc) הושהה. אומת ישירות: DNS ל-`sxjizqkeirvnsgxpixcc.supabase.co` לא נפתר (ENOTFOUND) בעוד `supabase.co` הראשי וגם `google.com` תקינים — כלומר לא בעיית רשת, הפרויקט אכן non-responsive. login לפרודקשן שבור עד ביטול השהיה מה-dashboard (חד-פעמי, ידני — לא ניתן מכאן). **בזמן ביטול ההשהיה — לסובב את `SUPABASE_SERVICE_KEY` שדלף.** נוסף keepalive cron (ראה session log 06-25) שאמור למנוע הישנות — עדיין דורש הגדרת `CRON_SECRET` ב-Vercel env vars (ידני, לא בוצע).
