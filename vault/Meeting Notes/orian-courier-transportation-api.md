...
- **`GetTransporttaionOrderLabel` נכשל פעמיים ברציפות**, גם עם `ORDERID=REFERENCEORDER` **תואם בדיוק** להזמנה שאומתה SUCCESS — תמיד אותה שגיאה: `400 Bad Request: Value cannot be null. Parameter name: s`. שתי התאוריות הקודמות **נפסלו**: (1) אי-התאמת ID. (2) דגל דחיסה (Make.com) — אותו דגל בדיוק היה גם ב-Create שהצליח. **`GetPackageStatus` על אותה חבילה הצליח במלואו (`PACKAGESTATUS:NEW`)** — מאשר שההזמנה קיימת ותקינה אצל Orian; הבעיה ממוקדת ספציפית ב-endpoint של התווית. הוצע ניסיון נוסף: לשלוח `ORDERID=PACKAGEID` (לא REFERENCEORDER) — אם גם זה נכשל, השלב הבא הוא פנייה ישירה לתמיכת Orian עם ה-repro המלא (כל שאר ה-endpoints שנבדקו — Login/Create/GetPackageStatus — עובדים מושלם).
