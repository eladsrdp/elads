...
- **`GetTransporttaionOrderLabel` נכשל פעמיים ברציפות**, גם עם `ORDERID=REFERENCEORDER` **תואם בדיוק** להזמנה שאומתה SUCCESS — תמיד אותה שגיאה: `400 Bad Request: Value cannot be null. Parameter name: s`. שתי התאוריות הקודמות **נפסלו**: (1) אי-התאמת ID — נפסלה, כי בניסיון השני ה-ORDERID תאם במדויק. (2) דגל דחיסה (`requestCompressedContent:true`) בכלי הבדיקה (Make.com) — נפסלה, כי אותו דגל בדיוק היה גם ב-`CreateTransportationOrder` שהצליח. כלי הבדיקה בפועל הוא **Make.com** (לא curl) — זה גם מסביר את "מודול 7" שהוזכר בשיחה קודמת (מספר מודול בתרחיש Make). המסקנה הנוכחית: כשל אמיתי/עקבי בקריאה הזו עצמה — או תקלה בסביבת ה-test של Orian לשליפת תוויות, או שהמסמך טועה וצריך internal ID ולא REFERENCEORDER (כמו שכבר קרה עם פורמט תגובת ה-Login). השלב הבא: `GetPackageStatus` על ה-PACKAGEID שנוצר, ואם תקין — לפנות לתמיכת Orian עם ה-repro המדויק.
