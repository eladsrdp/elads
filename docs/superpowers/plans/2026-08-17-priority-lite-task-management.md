...
## Task 1: סקריפט גילוי — אימות 3 הסיכונים הטכניים מול פריוריטי אמיתי
...
```ts
  custNoteFields: {
    ...
    // "לטיפול" — מועמד לפי מטא-דאטה, אושר/תוקן בסקריפט הגילוי (Task 1) לפני Task 8.
    handler: 'USERLOGIN',
  },
  /** תת-טופס תיאור מורחב ("תקציר המשימה") — ContainsTarget יחיד, שדה TEXT. */
  custNoteTextSubform: 'CUSTNOTESTEXT_ONE_SUBFORM',
...
**אם Task 1 גילה ש"לטיפול" הוא שדה אחר** (לא `USERLOGIN`) — עדכן רק את השורה `handler: 'USERLOGIN'` לשם השדה הנכון. שום קוד אחר לא צריך להשתנות (כל השימוש עובר דרך `mapping.ts` בלבד, בהתאם לכלל הקיים בראש הקובץ).
...
**חשוב:** לפני ה-task הזה, ודא ש-Task 1 (סקריפט הגילוי) רץ ותועד, ושה-`mapping.ts` (Task 3) מעודכן לפי הממצאים. אם משהו לא תואם למה שכתוב פה — עדכן את הקוד למטה בהתאם לממצאים, לא להפך.
